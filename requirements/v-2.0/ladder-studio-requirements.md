...
- Jump labels are emitted in clean form. The original's `CompileService.createLabel` baked the mnemonic into the label text itself, so a jump came out as `JNC JNC LABEL_x_y` and its target as `JNC LABEL_x_y :`. The port's `create_label` returns only `LABEL_x_y`, giving the intended `JNC LABEL_x_y` / `LABEL_x_y :` pair. This is a deliberate correction, not a preservation — the original's doubled form does not correspond to valid 8085 assembly — and will differ from old-tool output for any screen containing a jump.
